...
if (!customElements.get('product-bump-form')) {
    customElements.define('product-bump-form', class ProductBumpForm extends HTMLElement {
      ...
      onSubmitHandler(evt) {
        ...
        document.querySelector('#product-bump-form').innerHTML += '<input class="mainProductInput" type="hidden" name="items[0][id]" value="' + document.querySelector('#mainSelectedProduct').value + '"><input class="mainProductInput" type="hidden" name="items[0][quantity]" value="' + document.querySelector('input[name=quantity]').value +'">'
